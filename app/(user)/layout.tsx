...
export default function UserLayout({ children }: { children: React.ReactNode }) {
  ...
  const authError = isLoading
    ? null
    : !isAuthenticated
      ? 'Silakan login terlebih dahulu melalui halaman SITAMA untuk mengakses fitur ini.'
      : appRole !== 'user'
      ...
      <div className="flex h-screen items-center justify-center bg-gray-50">
        <div className="mx-4 max-w-md rounded-lg border border-red-200 bg-white p-6 text-center shadow-lg">
          ...
          <div className="flex justify-center">
            <a href={AUTH_URLS.USER_LOGIN} className="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
              Login
            </a>
          </div>
        </div>
